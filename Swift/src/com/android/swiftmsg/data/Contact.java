package com.android.swiftmsg.data;
...
public class Contact {
	...
	public static final int CONTACT_METHOD_TYPE_SELF = 3;   //"Me"
	...
	private long mContactMethodId;    //Id in phone or email Uri returned by provider of current
									  //Contact, -1 is invalid. e.g. contact method is 20 when 
	...
	private String mNameAndNumber;  //For display (John Smith <555-555-5555>)
	private boolean mNumberIsModified; //true if modified
	...
	private boolean mIsMe;          //True if = me
	private boolean mSendToVoicemail;     // true if this contact shouldn't put up notifications
	...
	/*
	 * Make a basic contact object with phone number
	 */
	...
	public String toString() {
		return String.format("{ number=%s, name=%s, nameAndNumber=%s, label=%s, person_id=%d, hash=%s method_id=%d }",
	...
	public static void logWithTrace(String tag, String msg, Object...objects format) {
	...
	/**
	 * Format the name and number.
	 * 
	 * @param name
	 * @param number
	 * @param numberE164 the number's E.164 represent the country num
	 *        belongs to
	 * @return the formated name and number
	 */
	public static String formatNAmeAndNumber(String name, String number, String numberE164) {
		//Format: John Smith <(555) 555-5555>
		String formattedNumber = number;
		if(!swiftmsg.isEmailAddress(number)) {
			formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164,
					swiftmsg.getApplication().getCurrentCountryIso());
		}
	...
	public synchronized boolean isEmail() {
		return swiftmsg.isEmailAddress(mNumber);
	}
	...
	private static class ContactsCache {
		...
		// query params for called id lookup without E164 number as param
		...
		private Contact get(String number, boolean isMe, boolean canBlock) {
			...
			if(TextUtils.isEmpty(number)) {
				number = "";             // In some places it's possible to receive
				                         // a message without the sender's address. In this case,
				                         // all such anon messages will get added to the same
				                         // thread.
			}
			
			// Always return a Contact object, if we don't have an actual contact
			...
			synchronized (contact) {
				...
				while (canBlock && contact.mQueryPending) {
					try {
						...
						// loop unless mQueryPending = false
					}
				}
				
				// If we're stale and we haven't already kick fof a query then kick
				// it off here.
			...
			/**
			 * Get CacheEntry list for given phone URIs. This method will do single on query to 
			...
			private boolean contactChanged(Contact orig, Contact newContactData) {
				// The number should never change, so don't bother checking.
				// TODO: maybe update it if it has gotten longer 6502345678 -> +16502345678
			...
			private void updateContact(final Contact c) {
				...
				synchronized (c) {
					if (contactChanged(c, entry)) {
						...
						// Prevents an endless loop from happening if updating an empty contact
						// Break the cycle and don't notify if the number is empty.
			...
			private Contact getContactInfo(Contact c) {
				...
				} else if (swiftmsg.isEmailAddress(c.mNumber) || isAlphaNumber(c.mNumber)) {
			...
			private Contact getContactInfoForPhoneNumber(String number) {
				...
				if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)){
					...
					String numberE164 = PhoneNumberUtils.formatNumberToE164(
							number, swiftmsgApp.getApplication().getCurrentCountryIso());
					String selection;
					String[] args;
			...
			private Contact getContactInfoForSelf() {
				...
				is (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
			...
			//Invert and truncate to five chars the phoneNumber so that we
			//can use it as the key in the hashtable. We keep a mapping of this
			...
			private String key(String phoneNumber, CharBuffer keyBuffer) {
				...
				if (resultcount > 0) {
					...
					// no usable digits in the input phoneNumber
					return phoneNumber;
				}
			}
			...
			private Contact internalGet(String numberOrEmail, boolean isMe) {
				synchronized (ContactsCache.this) {
					...
					final boolean isNotRegularPhoneNumber = isMe || swiftmsg.isEmailAddress(numberOrEmail) ||
			...
			private void remove(Contact contact) {
				synchronized (ContactsCache.this) {
					...
					final boolean isNotRegularPhoneNumber = contact.isMe() ||
							swiftmsg.isEmailAddress(number) ||
